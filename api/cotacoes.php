                ...
                try {
                    ...
                    $stmt = $conn->prepare("
                        SELECT ic.*,
                               ...
                               CASE 
                                   WHEN ic.valor_unitario > 0 AND ic.primeiro_valor > 0 
                                   THEN ((ic.valor_unitario - ic.primeiro_valor) / ic.primeiro_valor) * 100 
                                   ELSE 0 
                    ...
                    foreach ($itens as &$item) {
                        if (!empty($item['ultimo_valor_aprovado']) && $item['ultimo_valor_aprovado'] > 0) {
                            $valorAtual = floatval($item['valor_unitario']);
                            $valorAnterior = floatval($item['ultimo_valor_aprovado']);
                ...
                if (strpos($contentType, 'multipart/form-data') !== false) {
                    ...
                    try {
                        ...
                        foreach ($produtosMantidos as $produto) {
                            $placeholders[] = "(produto_nome = ? AND fornecedor_nome = ?)";
...
if ($dados['status'] === 'aprovado') {
    ...
    try {
        ...
        if ($sawingId) {
            ...
            $stmt = $conn->prepare("
                SELECT id, produto_nome, quantidade, valor_unitario, primeiro_valor, fornecedor_nome
                FROM itens_cotacao 
                WHERE cotacao_id = ? AND aprovado = 1
            ");
            $stmt->execute([$dados['id']]);
...
exit;
                        ...
                        if (isset($dados['fornecedores'])) {
                            ...
                            foreach ($dados['fornecedores'] as $fornecedor) {
                                ...
                                $stmt->execute([$dados['id'], $fornecedor['fornecedor_nome']]);
                                $result = $stmt->fetch(PDO::FETCH_ASSOC);
                                $produto_id = ($result['max_id'] ?? 0) + 1;
                                ...
                                foreach ($fornecedor['produtos'] as $produto) {
                                    ...
                                    if ($itemExiste) {
                                        ...
                                        // Primeiro, buscar o valor atual antes de atualizar
                                        ...
                                        $valorAtual = $stmtAtual->fetchColumn();
                                        
                                        // Agora atualizar com o valor anterior como ultimo_preco
                                        $stmt->execute([
                                            $produto['quantidade'],
                                            $produto['valor_unitario'],
                                            $produto['valor_total'],
                                            ...
                                            $fornecedor['difal'] ?? 0,
                                            $fornecedor['prazo_pagamento'] ?? null,
                                            $valorAtual, // Usar o valor atual como ultimo_preco
                                            $dados['id'],
                                            $produto['nome'],
                                            $fornecedor['fornecedor_nome']
                                        ]);
